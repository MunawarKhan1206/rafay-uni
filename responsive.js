...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (allSection) {
      ...
      // Limit the number of items to 30 and show them
      const allItems = allSection.querySelectorAll(".grid-item");
      allItems.forEach((item, index) => {
          if (index < MAX_ITEMS) {
              ...
              item.style.display = "none"; // Hide items beyond the 30th
          }
      });
  } else {
      console.log("All section NOT found! Check HTML.");
...
class Carousel {
  ...
  updateGallery() {
    ...
    // Limit the gallery to show 30 items at a time
    this.carouselArray.slice(0, 30).forEach((el, i) => {
      el.classList.add(`gallery-item-${i + 1}`);
    });
  }
